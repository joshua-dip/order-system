...
export function parseOrderRevenueFromOrderText(
  ...
  // 항목 옆 괄호 부연(예: "5. 가격 (고미조슈아 변형 제작·쏠북 커스텀 합산)") 또는 콜론 표기 변형(같은 줄 ":" / 다음 줄 ":")을 모두 흡수
  const patterns: RegExp[] = [
    ...
    /총\s*금액\s*[:：]?\s*([\d,]+)\s*원/,
    /기본\s*금액\s*[:：]?\s*([\d,]+)\s*원/,
    /* 번들(BB) 주문서는 「소계 / 합계 / 입금액」으로 적는다. 이 라벨들이 목록에 없어
       BB-20260831-001 의 20,900원이 화면에도 매출 집계에도 안 잡혔다.
       **소계는 넣지 않는다** — 할인 전 금액이라 실제 입금액보다 크다.
       줄머리에 오는 라벨만 받아 다른 문장 속 「합계」에 걸리지 않게 한다. */
    /(?:^|\n)\s*입금액\s*[:：]?\s*([\d,]+)\s*원/m,
    /(?:^|\n)\s*합계\s*[:：]?\s*([\d,]+)\s*원/m,
  ];
