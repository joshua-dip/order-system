#!/usr/bin/env bash
# [맥] 유형별 MLX LoRA 학습 — 주제·제목·주장·내용일치(fact)·빈칸(blank)·요약(summary) 공용. Claude/API 없음.
#   ./ml/common/train_mlx.sh <topic|title|claim|fact|blank|summary> [모델] [단계]
...
set -euo pipefail
TYPE="${1:?유형을 주세요: topic|title|claim|fact|blank|summary}"
case "${TYPE}" in topic|title|claim|fact|blank|summary) ;; *) echo "유형은 topic|title|claim|fact|blank|summary 중 하나: ${TYPE}" >&2; exit 1 ;; esac
