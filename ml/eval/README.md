...
**운영 상태(2026-09-27)**: main의 `03c6a0b`에는 요약 후보 코드가 포함되어 있다.
두 시험지 전체를 각 2회 전후 비교했으나 문장 번호로 근거를 받는 변경은 O% 차이가 모두 ±7%p 이내라 되돌렸다.
상품 사용 기준은 미달이며 운영 워커·호환 서버는 중지 상태를 유지한다. 먼저 운영을 재개한다면
요약 초안의 요지·문법·정답 유일성을 원문과 대조해 문항별로 직접 검수한다. 아래 학습·평가 명령은
운영 재개를 뜻하지 않으며, 이 방침을 강제하는 실행 차단 코드는 없다.

| 파일 | 내용 | 커밋 |
|---|---|---|
| `sets/<세트>.json` | 교재·지문 번호·유형, 지문별 채점 기준(`key`)과 알려진 함정(`pitfalls`). 원문은 DB 에서 읽는다 | O |
| `grades/<세트>/<label>.json` | 사람 또는 AI 채점(`grader` 명시) — `"20번\|topic": "O"` (O 맞음 · P 부분 · X 틀림 · F 실패) + 메모 | O |
...
- 운영 재개가 결정된 뒤에는 새 어댑터 적용을 위해 워커를 재시작한다(현재는 위 운영 중지 방침 우선):
