#!/usr/bin/env python3
"""GPU-free smoke check for Windows/Linux claim LoRA pipeline.
...
from _claim_common import extract_json_object, SYSTEM_PROMPT  # noqa: E402


def main() -> int:
    errors: list[str] = []

    if "주장" not in SYSTEM_PROMPT or "CorrectAnswer" not in SYSTEM_PROMPT:
        errors.append("SYSTEM_PROMPT invalid")

    sample = '{"Question":"이 글의 주장으로 가장 적절한 것은?","Paragraph":"Hello.","Options":"① a ### ② b ### ③ c ### ④ d ### ⑤ e","CorrectAnswer":"①","Explanation":"테스트","OptionType":"English"}'
    ...
    data = _ROOT / "data" / "claim-finetune" / "train.jsonl"
    if data.is_file():
        n = sum(1 for line in data.open(encoding="utf-8") if line.strip())
        print(f"ok: train.jsonl {n} lines")
    else:
        print("info: train.jsonl missing - run npm run cc:claim-export")

    import ast

    for name in ("train.py", "infer.py", "pipeline_claim.py", "_cuda_runtime.py"):
    ...
    docs = _ROOT / "docs" / "ml" / "claim-windows-cuda.md"
    if not docs.is_file():
        errors.append("docs/ml/claim-windows-cuda.md missing")
