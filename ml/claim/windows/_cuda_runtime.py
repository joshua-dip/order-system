"""Shared CUDA model load + chat generate for Windows claim LoRA."""
...
if str(_TYPE_DIR) not in sys.path:
    sys.path.insert(0, str(_TYPE_DIR))

from _claim_common import extract_json_object  # noqa: E402

DEFAULT_ADAPTER = _TYPE_DIR / "adapters" / "claim-lora-cuda"
DEFAULT_EXPLAIN_ADAPTER = _TYPE_DIR / "adapters" / "claim-explain-lora-cuda"
...
def load_model(
    ...
    try:
        ...
        raise RuntimeError(
            f"transformers/peft missing ({e}). Run ml\\claim\\windows\\setup.bat"
        ) from e
