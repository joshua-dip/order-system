#!/usr/bin/env python3
"""Windows NVIDIA CUDA — 주장 유형 LoRA/QLoRA 학습.

Mac MLX 와 같은 data/claim-finetune/{train,valid}.jsonl (chat messages) 를 쓴다.
...
DATA_DIR = ROOT / "data" / "claim-finetune"
DEFAULT_ADAPTER = Path(__file__).resolve().parent.parent / "adapters" / "claim-lora-cuda"


def main() -> int:
    ap = argparse.ArgumentParser(description="Claim variant LoRA train (Windows CUDA)")
    ap.add_argument(
        "--model",
    ...
    try:
        ...
        print(
            f"의존성 없음: {e}\n"
            "ml\\claim\\windows\\setup.bat 을 먼저 실행하세요.",
            file=sys.stderr,
        )
        return 1
