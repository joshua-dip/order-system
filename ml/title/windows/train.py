#!/usr/bin/env python3
"""Windows NVIDIA CUDA — 제목 유형 LoRA/QLoRA 학습.

Mac MLX 와 같은 data/title-finetune/{train,valid}.jsonl (chat messages) 를 쓴다.
...
DATA_DIR = ROOT / "data" / "title-finetune"
DEFAULT_ADAPTER = Path(__file__).resolve().parent.parent / "adapters" / "title-lora-cuda"


def main() -> int:
    ap = argparse.ArgumentParser(description="Title variant LoRA train (Windows CUDA)")
    ap.add_argument(
        "--model",
    ...
    try:
        ...
        print(
            f"의존성 없음: {e}\n"
            "ml\\title\\windows\\setup.bat 을 먼저 실행하세요.",
            file=sys.stderr,
        )
        return 1
