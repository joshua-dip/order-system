#!/usr/bin/env python3
"""순서(order) 파이프라인 — LoRA 없는 규칙 유형. 규칙 생성 → 35B 모의 풀이로 거르기 → 해설(ml/common/rule_pipeline.py).
...
def run_pipeline(model: Any, tokenizer: Any, passage: str, *, max_retries: int = 2, temp: float = 0.3,
    ...
    return rule_pipeline.run("order", chat_text, model, tokenizer, passage, temp=temp)
