#!/usr/bin/env python3
"""무관한문장(irrelevant) 파이프라인 — LoRA 없는 유형. 자리는 코드가, 바꿀 것 하나는 35B 가 → 35B 모의 풀이로 거르기 → 해설
...
def run_pipeline(model: Any, tokenizer: Any, passage: str, *, max_retries: int = 2, temp: float = 0.3,
    ...
    return edit_pipeline.run("irrelevant", chat_text, model, tokenizer, passage, temp=temp)
