...
- `Explanation` 은 한국어 **180~450자**, 결론 하나만. 한 줄 해설 금지.
...
| 유형 | 목표 글자 수 | 반드시 넣을 것 |
|------|-------------|----------------|
| 제목·주제·주장·일치·불일치 | 180~300 | 정답 번호 시작 + 요지/근거 + 정답 대응 + **오답 1~2개 배제** |
| 함의 | 250~400 | 표면 의미 → 맥락 함의 → 정답 대응 (정형구 금지) |
| 빈칸·요약 | 180~350 | 빈칸/요약문이 글 논리에서 하는 역할 + 오답 배제 |
| 순서·삽입 | ~450 이하 | 번호 시작 + 앞뒤 연결 근거 + 「논리 흐름 요약:」 |

**나쁜 예(금지):** `"같은 옷 위장에도 속지 않았다는 점에서 ④이 핵심을 함축한다."`  
**좋은 예 골격:** `"⑤가 정답입니다. 글은 …(원문 구절)…라고 하여 …. 따라서 …라는 제목이 글 전체를 아우릅니다. ①은 …해서 글과 반대이고, ③은 …만 잡아 지엽적입니다."`

권위 프롬프트: [`lib/admin-variant-draft-claude.ts`](../../lib/admin-variant-draft-claude.ts) 공통 규칙 5)·유형별 Explanation.
