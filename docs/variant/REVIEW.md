...
npm run cc:answer-seq -- bundle   <주문번호> [--insert] [--relaxed] [--apply]    # 통합본(지문 → 유형 인쇄 순서)
```
...
- **통합본으로 나가는 주문은 통합본 순서로 본다.** `cc:order-pdf --by round` 는 한 지문의 유형을 수능 문항 번호 순
  (함의→어법→어휘→빈칸→순서→삽입→요약)으로 이어 붙이므로 이웃이 다른 유형이다. `bundle` 이 같은 인쇄 순서로 단일 정답만 이어 보고
  셔플형 → 순서 → 삽입(`--insert`) 순으로 옮긴다. 회차가 없는 모의고사는 교재 하나가 통합본 하나다(연도별 통합본).
- **삽입-고난도는 정답 ① 금지**(`lib/hard-insertion-generator.ts`). `insert`·`bundle --insert` 는 ①로 옮기지 않고, ①이면 옮긴다.
  브릿지 문장이 첫 문장 바로 뒤에 붙는 문항은 `--relaxed` 로 첫 문장 앞에 ① 마커를 둔다 — 지시어로 앞 문장을 받는 브릿지는
  글 첫머리에 올 수 없어 ①이 분명한 오답이 된다. 병렬 샤드에 삽입 정답을 미리 배정할 때도 ①은 주지 않는다.
