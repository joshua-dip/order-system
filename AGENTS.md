...
- **Codex Pro / Max** = Codex **Code·채팅** 안에서 쓰는 모델 사용량 (여기서 문제 지문 읽고 JSON 직접 작성).
...
- **쓰지 않기:** MCP `variant_generate_draft` / `variant_generate_and_save`, 관리자 웹 **「Codex로 초안 생성」**, `POST /api/cron/variant-auto-fill`, `npm run variant:auto-fill` (전부 API 키).
...
권위는 코드(`lib/admin-variant-draft-Codex.ts` · `variant-draft-*-rules.ts`)이고,
...
다른 컴퓨터의 세션(제작기 `프로젝트9 - Codex`, WSL)과 주고받은 합의 문서는
...
- **BV·MV·주문 기준 부족 분:** **`variant_get_shortage`에 `orderNumber`만 전달** (부교재 변형 BV, 모의고사 변형 MV 모두 `orderMeta.flow`에 따라 집계). MCP 없이면 `npm run cc:variant -- shortage --order-number MV-…` / `BV-…`. **더 짧게:** `npm run Codex -- Codex:MV-20260401-003` 등 주문번호만 써도 `shortage`와 동일 JSON.
...
**터미널만 (MCP 없이):** `npm run cc:variant -- --help` · **주문 한 방 (shortage만)**: `npm run Codex -- Codex:BV-20260331-002` · **주문 한 줄 파이프라인 (검수까지)**: `npm run cc:variant -- pipeline:BV-20260529-002` 또는 단축 `npm run cc:variant -- BV-…` · 프로젝트 루트에서 실행 권한이 있으면 `./Codex Codex:BV-…` 도 동일하게 동작 (`Codex` 스크립트가 `tsx`로 CLI를 호출).
...
- 저장분은 `source_file='Codex'`·`excel_row_status='Codex-authored'` 마커로 엑셀 임포트 curated 분과 구분.
...
**한 줄 호출용 워크플로우 프롬프트** (`Codex agents` 의 task description 같은 single-line 입력에서도 안전) — `Codex agents`/`Codex` 채팅에서 매번 멀티라인 프롬프트를 paste 하면 truncate 위험이 있으니 다음을 첨부:
...
```bash
# 한 교재 자동 채움 — 현재 터미널에서 Codex 띄우고 ScheduleWakeup 10분 루프
...
./scripts/run-essay-loop-multi.sh \
  ...
  "24년 6월 고1 영어모의고사"
```
내부에서 `Codex --dangerously-skip-permissions "@scripts/cc-essay-loop-prompt.md …"` 로 실행. **에이전트가 룰을 안 어기는 한 안전**하지만, 잘못 만든 명령도 즉시 실행되므로 새 워크플로우 검증할 때는 일반 `Codex` 로 먼저 1 cycle 확인.
...
## 출제 스튜디오 출제 포인트 (Pro 전용 — `cc:studio`)

`/my/vip/studio` 의 **출제 포인트(marks)** 를 채팅에서 같이 잡아 `vip_studio` 에 저장. **API 키 호출 없음** — 지문 원문을 받아 채팅에서 marks JSON 작성 → CLI 가 위치 자동 고정·검증·upsert. (스튜디오 = (userId,passageId) 1문서. 객관식 변형 DB·서술형과 별개. problems 는 안 건드림.)

```
npm run cc:studio -- exams      --user <email|loginId|이름> [--school "..."] [--grade 1]
npm run cc:studio -- exam       --id <examId>                    # 시험범위 지문 + passageId
npm run cc:studio -- find       --textbook "..." [--source "..."] # 모의고사 지문 passageId
npm run cc:studio -- passage    --id <passageId> [--user <...>]   # 원문+문장표+현재 마크+owners+어법 N/5
npm run cc:studio -- export     <passageId> --user <...>          # 현재 marks 를 save JSON 으로 덤프
npm run cc:studio -- save-marks --json draft.json [--dry-run] [--passage-id <pid>] [--user <...>] [--mode merge|replace]
단축: npm run cc:studio -- <passageId>   → passage --id <passageId>
```

- **작성 흐름**: ① `exam --id`/`find` 로 passageId 찾기 → ② `passage --id --user` 로 원문·현재 마크 받기 → ③ 채팅에서 출제 포인트 marks JSON 작성 → ④ `save-marks --dry-run` 검증 → ⑤ `save-marks` 로 upsert. 웹 스튜디오에 즉시 반영.
...
- **금지**: 웹 안 라이브 Codex 챗/서버 API 호출(ANTHROPIC 과금). 출제 포인트도 CLI `save-marks` 만 사용.
