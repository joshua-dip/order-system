...
const QuestionSettings = ({
    ...
    fetchAuthMe()
      .then((d) => {
        ...
        fetch('/api/my/variant-base-quota', { credentials: 'include' })
          .then((r) => r.json())
          .then((q) => {
            setBaseQuotaRemaining(Number(q?.remaining) || 0);
  ...
  const renderStandardTypeCard = (type: string) => {
    const locked = isFreeType(type) && !hasPaidType(selectedTypes) && !selectedTypes.includes(type);
    return (
                <div
                  key={type}
  ...
  const selectableTypes = questionTypes;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
                ...
                {isExternal ? (
                  ...
                  <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">
                    ...
                    <span className="block text-[12px] text-slate-500 mt-0.5">
                      쏠북 교재는 무료 문항(무료 유형·멤버십 무료 한도)이 적용되지 않습니다. 모든 유형을 주문제작 단가로 받고, 변형문제 금액은 쏠북에서 결제합니다.
                    </span>
                  </div>
                ) : isPremiumMembership ? (
                  ...
                  <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                    👑 <b>월·연회원</b>은 기본난도를 <b>월 1,000문항까지 무료</b>로 받습니다.
                  </div>
                )}
              </div>
              ...
              <div className="mb-6">
                ...
                {/* 무료 유형 — 유료를 하나 이상 골라야 열린다. 외부지문·쏠북 교재엔 무료 유형이 없어(isFreeType) 감춰진다 */}
