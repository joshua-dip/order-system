...
function view(d: WritingDoc, fmt: 'mc' | 'subjective') {
  const useSubj = fmt === 'subjective' && d.subjective;
  return {
    serialNo: d.serialNo ?? null,
    serial: formatWritingSerial(d.serialNo),
    topicKey: d.topicKey ?? '',
    course: d.과정 ?? '',
    chapter: d.대단원 ?? '',
    topic: d.학습주제 ?? '',
...
/**
 * GET /api/public/writing-bank — 라이팅 문제은행을 외부 웹에서 사용 (문제은행 API 와 같은 키).
...
export async function GET(request: NextRequest) {
  ...
  const col = db.collection<WritingDoc>(WRITING_PROBLEMS_COLLECTION);
  ...
  if (meta === 'topics') {
    ...
    // 무작위 표본 — 문제지 출제용
    const docs = await col.aggregate<WritingDoc>([{ $match: filter }, { $sample: { size: sample } }]).toArray();
