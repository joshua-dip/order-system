...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const withUrls = await Promise.all(photos.map(async (p) => {
    ...
    return { id: String(p._id), questionNum: p.questionNum, slotType: p.slotType ?? 'question', name: p.name, url, uploadedAt: p.uploadedAt };
  }));

  // slotType 으로 분리: byNum(문항별·레거시), byPassage(지문별)
  const byNum: Record<string, typeof withUrls> = {};
  const byPassage: Record<string, typeof withUrls> = {};
  for (const p of withUrls) {
    const bucket = p.slotType === 'passage' ? byPassage : byNum;
    (bucket[p.questionNum] ||= []).push(p);
  }
  return NextResponse.json({ ok: true, byNum, byPassage, total: withUrls.length });
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const slotType: 'question' | 'passage' = form.get('slotType') === 'passage' ? 'passage' : 'question';
  // 지문 키(`교재::sourceKey`)는 문항 번호보다 길어 한도를 넉넉히
  const maxKey = slotType === 'passage' ? 200 : 12;
  const questionNum = String(form.get('questionNum') ?? '').trim().slice(0, maxKey);
  if (!file) return NextResponse.json({ error: '파일이 없습니다.' }, { status: 400 });
  if (!questionNum) return NextResponse.json({ error: slotType === 'passage' ? '지문 키가 없습니다.' : '문항 번호가 없습니다.' }, { status: 400 });
  ...
  try {
    ...
    const doc: ExamQuestionPhotoDoc = {
      userId, examId: new ObjectId(id), questionNum, slotType, dropboxPath: path, name, uploadedAt: new Date(),
    };
    const r = await db.collection(EXAM_PHOTOS_COLLECTION).insertOne(doc);
    return NextResponse.json({ ok: true, photo: { id: String(r.insertedId), questionNum, slotType, name, url: tempUrl } });
  } catch (e) {
