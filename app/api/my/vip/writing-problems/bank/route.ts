...
/**
 * GET — 라이팅 문제은행 조회 (소유자 스코프)
 *  · ?topicKey=... [&source=] [&category=객관식|주관식|선택형] → 해당 진도의 문제 목록
 *  · 파라미터 없음 [&source=] [&category=] → topicKey별 문제 수 맵 + 출처 목록 + 카테고리 집계
 */
    ...
    problems: docs.map((d) => ({
      ...
      subjective: d.subjective && typeof d.subjective === 'object'
        ? {
            ...
            choices: Array.isArray((d.subjective as Record<string, unknown>).choices) ? ((d.subjective as Record<string, unknown>).choices as string[]) : undefined,
          }
        : undefined,
    })),
  });
