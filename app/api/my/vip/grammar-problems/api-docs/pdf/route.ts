...
async function handleGET(request: NextRequest) {
  ...
  const endpoint = `${origin}/api/public/grammar-bank`;
  ...
  const params: Array<[string, string, string]> = [
    ...
    ['topicKey', '문자열', '진도 키 정확 일치. 예) 중등 영문법 LEVEL 1 &gt; 조동사 &gt; can, may'],
    ['course / chapter / topic', '문자열', '과정·대단원·학습주제 개별 필터 (topicKey 없을 때)'],
    ['source', '문자열', '출처 필터. 예) 자체 제작 LEVEL 1'],
  ...
  '${endpoint}?' + new URLSearchParams({
    course: '중등 영문법 LEVEL 1', topic: 'can, may',
    format: 'mc', limit: '20',
  }),
...
r = requests.get("${endpoint}",
  headers={"Authorization": "Bearer ${sampleKey}"},
  params={"course": "중등 영문법 LEVEL 1", "topic": "can, may", "sample": 10})
for q in r.json()["items"]:
  ...
  const html = `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><style>
    * { box-sizing: border-box; }
    ...
    <h1>문법 문제은행 API</h1>
    <p class="sub">다른 웹·앱에서 문법 문제은행을 불러오는 공개 REST API 공식 문서입니다.</p>
  ...
  try {
    ...
    const name = encodeURIComponent('문법-문제은행-API-문서.pdf');
...
export const GET = withDownloadTracking('VIP 문법 API 문서', handleGET);
