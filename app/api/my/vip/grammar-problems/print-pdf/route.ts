...
async function handleGET(request: NextRequest) {
  ...
  const withConcepts = sp.get('concepts') === '1';

  const db = await getDb('gomijoshua');
  const blocks = await fetchWorksheetBlocks({
    db, collection: GRAMMAR_PROBLEMS_COLLECTION, userId: auth.userId, keys, source, category, formatSerial: formatGrammarSerial,
  });
  let concepts: Record<string, WsConcept> | undefined;
  if (withConcepts) {
    const rows = await db.collection(GRAMMAR_CONCEPTS_COLLECTION)
      .find({ userId: new ObjectId(auth.userId), topicKey: { $in: keys } }).toArray();
    concepts = {};
  ...
  const html = buildWorksheetHtml(blocks, { fmt, withAnswers, concepts });
...
export const GET = withDownloadTracking('VIP 문법 학습지', handleGET);
