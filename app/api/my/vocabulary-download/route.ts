...
export async function POST(request: NextRequest) {
  ...
  if (!user || !hasAnnualMemberMenuAccess({ annualSince: since ?? null, monthlyUntil: monthlyUntil ?? null, signupPremiumTrialUntil: trialUntil ?? null })) {
    return NextResponse.json({ error: '월·연 회원만 이용할 수 있습니다.' }, { status: 403 });
  }
