...
/**
 * Explanation 필드에 'API' 텍스트가 포함된 문항 검증.
 * 교재·유형 필터 적용, 스캔 건수/매칭 건수 및 목록 반환.
 */
export async function GET(request: NextRequest) {
  ...
  const match: Record<string, unknown> = {
    'question_data.Explanation': { $regex: '\\bAPI\\b', $options: 'i' },
  };
  ...
  try {
    ...
    const items = docs.map((d) => {
      const expl = (d.question_data as Record<string, unknown>)?.Explanation;
      const str = typeof expl === 'string' ? expl : '';
    ...
    const totalScanned = totalMatched;
    ...
    console.error('validate/explanation-api:', e);
    return NextResponse.json(
      { error: 'Explanation API 검증 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
