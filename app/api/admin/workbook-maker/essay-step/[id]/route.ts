...
import {
  deleteEssayStepWorkbook,
  getEssayStepWorkbook,
  updateEssayStepWorkbook,
...
export async function GET(request: NextRequest, ctx: Ctx) {
  ...
  const doc = await getEssayStepWorkbook(id);
...
export async function PATCH(request: NextRequest, ctx: Ctx) {
  ...
  const ok = await updateEssayStepWorkbook(id, patch);
  if (!ok) return NextResponse.json({ error: '문서를 찾을 수 없습니다.' }, { status: 404 });
...
export async function DELETE(request: NextRequest, ctx: Ctx) {
  ...
  if (error) return error;
  const { id } = await ctx.params;
  const ok = await deleteEssayStepWorkbook(id);
