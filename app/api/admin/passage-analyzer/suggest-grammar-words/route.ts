  ...
  try {
    ...
    const promptText =
      typeof customPrompt === 'string' && customPrompt.trim()
        ? buildGrammarAiUserContent(sentences, customPrompt)
        : buildGrammarAiUserContent(sentences, DEFAULT_GRAMMAR_AI_PROMPT);
    ...
    const parsed = JSON.parse(jsonText) as { selections?: GrammarAiSelectionRow[] };
    const selections = Array.isArray(parsed.selections) ? parsed.selections : [];
    const wordKeys = grammarAiJsonToWordKeys(sentences, selections);

    return NextResponse.json({ success: true, wordKeys, selections });
  } catch (e) {
    const errMsg = e instanceof Error ? e.message : String(e);
    console.error('suggest-grammar-words:', e);
    return NextResponse.json({ error: errMsg || '어법 AI 분석 실패' }, { status: 500 });
  }
}
