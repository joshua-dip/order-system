...
export default function AdminGeneratedQuestionsPage() {
  ...
  /** 로컬 LoRA 작업(GPU PC 워커가 처리) — 등록부터 완료까지 폴링한다. 주제·제목·주장·일치·불일치·빈칸·요약·순서·삽입·무관한문장·어휘·어법만 */
  ...
  const runGenerateDraftLocal = async () => {
    ...
    if (!isLocalVariantType(type)) {
      setDraftError('로컬 LoRA는 주제·제목·주장·일치·불일치·빈칸·요약·순서·삽입·무관한문장·어휘·어법 유형만 지원합니다.');
      return;
    }
    const pid = form.passage_id.trim();
    if (!/^[a-f0-9]{24}$/i.test(pid)) {
