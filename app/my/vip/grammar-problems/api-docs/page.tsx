...
export default function GrammarApiDocsPage() {
  ...
  const endpoint = `${origin || 'https://<도메인>'}/api/public/grammar-bank`;
  const sampleKey = keys?.[0]?.key ?? '<API_KEY>';
  const exTopicKey = '중등 영문법 LEVEL 1 > 조동사 > can, may';
...
for (const q of data.items) {
  console.log(q.serial, q.question);   // G-000xxx / 문항(밑줄은 <u>..</u>)
...
for q in data["items"]:
  ...
  "items": [
    {
      "serial": "G-000060",
      "topicKey": "중등 영문법 LEVEL 1 > 조동사 > can, may",
      "course": "중등 영문법 LEVEL 1",
      "chapter": "조동사",
      "topic": "can, may",
      "unit": 10,
  ...
  const params: Array<[string, string, string]> = [
    ...
    ['topicKey', '진도 키 정확 일치. 예) "중등 영문법 LEVEL 1 > 조동사 > can, may"', '문자열'],
    ['course', '과정(레벨) 필터. 예) "중등 영문법 LEVEL 1"', '문자열'],
    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <h1 className="mt-1.5 text-2xl font-bold text-zinc-100">문법 문제은행 API</h1>
          <p className="text-sm text-zinc-500 mt-0.5">다른 웹·앱에서 문법 문제은행을 불러오는 공개 REST API 공식 문서입니다.</p>
        </div>
        <div className="no-print flex shrink-0 items-center gap-2">
          <a
            href="/api/my/vip/grammar-problems/api-docs/pdf"
          ...
          <a href="/my/vip/grammar-problems" className="rounded-lg border border-zinc-700 bg-zinc-900/60 px-3 py-2 text-sm text-zinc-300 hover:border-zinc-500">← 문법 문제관리</a>
        </div>
      </div>
