...
/** 지문별 유형 고정에서 고를 수 있는 변형 유형(기본형 — 서버가 고난도형까지 확장) */
const PIN_TYPE_OPTIONS = ['주제', '제목', '주장', '빈칸', '순서', '삽입', '무관한문장', '어법', '어휘', '요약', '함의', '일치', '불일치'];
...
export default function GenerateExamForm({ forcedMode }: { forcedMode: Mode }) {
  ...
  /* 지문별 유형 고정 — { "교재::source": "유형" }. '이 지문은 이 유형으로만' */
  const [passagePins, setPassagePins] = useState<Record<string, string>>({});
  ...
  /* 지문별 유형 고정 UI — scope 소스를 카테고리(교과서 먼저)로 그룹 */
  const pinGroups = useMemo<{ cat: SourceCategory; items: { tb: string; src: string }[] }[]>(() => {
    const m: Record<SourceCategory, { tb: string; src: string }[]> = { 교과서: [], 부교재: [], 모의고사: [] };
    for (const { tb, sources } of scopeSourcesByTb) {
      const cat = classifyCategory(tb);
      if (sources.length === 0) m[cat].push({ tb, src: '' });
      else for (const src of sources) m[cat].push({ tb, src });
    }
    return CATEGORY_ORDER.filter((c) => m[c].length > 0).map((c) => ({ cat: c, items: m[c] }));
  }, [scopeSourcesByTb, classifyCategory]);
  ...
  /* 범위 시험이 바뀌면 지문 고정 초기화 (지문 목록이 달라짐) */
  useEffect(() => { setPassagePins({}); setPinsExpanded(false); }, [scopeExamId]);

  /* 범위 지문별 보유 변형 유형 로드 */
  useEffect(() => {
    if (mode !== 'school' || !scopeExamId) { setPassageTypeAvail({}); return; }
    let alive = true;
    fetch(`/api/my/vip/generate/scope-passage-types?scopeExamId=${scopeExamId}`, { credentials: 'include' })
      .then((r) => r.json())
      .then((d) => { if (alive) setPassageTypeAvail(d.ok && d.types ? d.types : {}); })
      .catch(() => { if (alive) setPassageTypeAvail({}); });
    return () => { alive = false; };
  }, [mode, scopeExamId]);
  ...
  const pinnedCount = Object.values(passagePins).filter((v) => v && v !== '자동').length;
        ...
        <div className="rounded-xl bg-zinc-900/50 border border-zinc-800/80 p-5 space-y-5">
          ...
          {pinGroups.length > 0 && (
            <div className="rounded-xl border border-zinc-800/80 bg-zinc-900/30">
              <button type="button" onClick={() => setPinsExpanded((v) => !v)}
                className="w-full flex items-center justify-between px-4 py-2.5 text-left">
                <span className="text-xs font-medium text-zinc-300">
                  지문별 유형 고정 <span className="text-zinc-600">(선택 · 지정 지문은 그 유형으로만 출제)</span>
                </span>
                <span className="flex items-center gap-2">
                  {pinnedCount > 0 && <span className="px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-300 text-[10px]">{pinnedCount}개 고정</span>}
                  <span className="text-zinc-500 text-xs">{pinsExpanded ? '접기 ▲' : '펼치기 ▼'}</span>
                </span>
              </button>
              {pinsExpanded && (
                <div className="px-4 pb-3 space-y-3 max-h-72 overflow-y-auto">
                  <p className="text-[10px] text-zinc-500 leading-relaxed">
                    특정 지문을 반드시 지정 유형으로 출제합니다. (한 시험지 안에서 같은 지문은 한 번만 쓰이므로 그 지문은 그 유형으로만 나옵니다. 기본형 지정 시 고난도형까지 후보에 포함됩니다.)
                  </p>
                  {pinGroups.map(({ cat, items }) => (
                    <div key={cat}>
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className={`text-[11px] font-semibold ${cat === '교과서' ? 'text-emerald-300' : cat === '모의고사' ? 'text-sky-300' : 'text-orange-300'}`}>{cat}</span>
                        <div className="flex-1 h-px bg-zinc-800" />
                      </div>
                      <div className="space-y-1.5">
                        {items.map(({ tb, src }) => {
                          const key = `${tb}::${src}`;
                          const avail = passageTypeAvail[key];
                          const loaded = Object.keys(passageTypeAvail).length > 0;
                          // 보유 유형만 노출 — 기본형은 그 고난도형이 있어도 선택 가능(서버가 폴백)
                          const opts = (loaded && avail)
                            ? PIN_TYPE_OPTIONS.map((t) => ({ t, n: (avail[t] || 0) + (avail[`${t}-고난도`] || 0) })).filter((o) => o.n > 0)
                            : PIN_TYPE_OPTIONS.map((t) => ({ t, n: 0 }));
                          const noVariants = loaded && !!avail && opts.length === 0;
                          return (
                            <div key={key} className="flex items-center gap-2">
                              <div className="flex-1 min-w-0">
                                <p className="text-[11px] text-zinc-300 truncate" title={`${tb} · ${src}`}>{src || '(번호 미상)'}</p>
                                <p className="text-[10px] text-zinc-600 truncate">{tb}</p>
                              </div>
                              <select disabled={noVariants} value={passagePins[key] ?? ''} onChange={(e) => setPassagePins((p) => { const n = { ...p }; if (!e.target.value) delete n[key]; else n[key] = e.target.value; return n; })}
                                className={`px-2 py-1 rounded-lg border text-[11px] focus:outline-none disabled:opacity-40 [&>option]:bg-zinc-900 ${passagePins[key] ? 'bg-emerald-900/30 border-emerald-700/50 text-emerald-200' : 'bg-zinc-900/60 border-zinc-800/80 text-zinc-400'}`}>
                                <option value="">{noVariants ? '변형 없음' : '자동'}</option>
                                {opts.map(({ t, n }) => <option key={t} value={t}>{n > 0 ? `${t} (${n})` : t}</option>)}
                              </select>
                            </div>
                          );
                        })}
