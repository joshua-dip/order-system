...
import { WRITING_CURRICULA, WRITING_LEVELS, writingTopicKey, countTopics, countBigunitTopics, type WritingCurriculum } from '@/lib/writing-curriculum';
import { questionHtml, answerLabel, problemView, CIRCLED_NUMS, categoryBadgeClass, type BankProblem, type BankFormat } from '../grammar-problems/shared';
...
export default function WritingProblemsPage() {
  const [courseName, setCourseName] = useState(WRITING_CURRICULA[0]?.과정 ?? '');
  const curriculum: WritingCurriculum = useMemo(
    () => WRITING_CURRICULA.find((c) => c.과정 === courseName) ?? WRITING_CURRICULA[0],
    [courseName],
  );
  ...
  const [fmt, setFmt] = useState<BankFormat>('mc');
  ...
  useEffect(() => {
    ...
    fetch(`/api/my/vip/writing-problems/bank${qs ? `?${qs}` : ''}`, { credentials: 'include' })
      .then((r) => r.json())
      .then((d) => {
  ...
  useEffect(() => {
    ...
    Promise.all(
      selectedTopics.map((t) =>
        fetch(`/api/my/vip/writing-problems/bank?topicKey=${encodeURIComponent(t.key)}${source ? `&source=${encodeURIComponent(source)}` : ''}${category ? `&category=${encodeURIComponent(category)}` : ''}`, { credentials: 'include' })
          .then((r) => r.json())
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">라이팅 진도(레벨·목차)에서 학습주제를 고르면, 해당 진도의 문제를 생성·관리합니다</p>
        </div>
        <a
          href="/my/vip/writing-problems/api-docs"
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select
          ...
          onChange={(e) => {
            ...
            setCourseName(name);
            setSelectedTopics([]);
            const first = WRITING_CURRICULA.find((c) => c.과정 === name)?.대단원[0]?.대단원명 ?? '';
        ...
        >
          {WRITING_LEVELS.map((lv) => {
            const items = WRITING_CURRICULA.filter((c) => c.레벨 === lv);
                    ...
                    {directTopics.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 py-1">
                        {directTopics.map((topic) => {
                          const key = writingTopicKey(curriculum.과정, big.대단원명, '', '', '', topic);
                            ...
                            <div className="mt-0.5 ml-2 space-y-2 pb-1">
                              {mid.소단원.map((sub) => {
                                ...
                                const chip = (topic: string) => {
                                  const key = writingTopicKey(curriculum.과정, big.대단원명, mid.중단원명, sub.소단원명, '', topic);
                                  const active = isSelected(key);
                                  return (
                                    <button
                                      key={topic}
                ...
                <div className="mt-1.5 flex flex-wrap gap-1.5">
                  {selectedTopics.map((t) => (
                    <span key={t.key} className="inline-flex items-center gap-1 rounded-md border border-[#c9a44e]/40 bg-[#c9a44e]/10 px-2 py-1 text-[12px] text-[#e8d48b]">
                      ...
                      {t.학습주제}
                      <button type="button" onClick={() => toggleTopic(t)} className="ml-0.5 text-zinc-400 hover:text-rose-300" aria-label="제거">×</button>
                    </span>
                  ))}
                </div>
              </div>
      ...
      {showPrint && (
        <PrintWorksheet
          ...
          source={source}
          category={category}
          bankBase="/api/my/vip/writing-problems/bank"
          initialFmt={fmt}
