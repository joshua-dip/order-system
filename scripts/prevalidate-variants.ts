...
async function main() {
  ...
  for (const it of items) {
    ...
    if (it.type !== '어법-고난도' && !CIRCLED.includes(String(q.CorrectAnswer) as typeof CIRCLED[number])) {
      errs.push(`${tag} CorrectAnswer 형식: ${q.CorrectAnswer}`);
    }

    const opts = String(q.Options ?? '').split('###').map((s) => s.trim()).filter(Boolean);
    const needFiveOptions = ['빈칸-고난도', '함의-고난도'].includes(it.type);
    if (needFiveOptions) {
      ...
      if (it.type === '빈칸-고난도' && ans && norm(P).includes(ans)) errs.push(`${tag} 정답 문구가 본문에 노출됨`);
    }
    ...
    if (it.type === '빈칸-고난도' && !/_{5,}/.test(P)) errs.push(`${tag} 빈칸 표식 없음`);
