/**
 * 주제 해설 전용 파인튜닝 JSONL export (Claude/Anthropic 없음).
 *
 * 입력: 지문 + Options + CorrectAnswer
 * 출력: Explanation 만 (한국어)
 *
 *   npm run cc:topic-explain-export
 *   npm run cc:topic-explain-export -- --status 완료 --out data/topic-explain-finetune
 */
...
const EXPLAIN_SYSTEM = `당신은 한국 수능 영어 「주제」 문항의 한국어 해설만 작성합니다.
...
async function main() {
  ...
  const outDir = path.resolve(PROJECT_ROOT, flags.get('out') || 'data/topic-explain-finetune');
