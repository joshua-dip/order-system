/**
 * 주장 유형 파인튜닝용 JSONL export (Claude/Anthropic 호출 없음).
 *
 *   npm run cc:claim-export
 *   npm run cc:claim-export -- --status 완료 --out data/claim-finetune
 *
...
const SYSTEM_PROMPT = `당신은 한국 수능 영어 변형문제 출제자입니다. 주어진 영어 지문으로 「주장」 객관식 1문항을 만듭니다.
...
function assistantPayload(qd: Record<string, unknown>, paragraph: string): string {
  const body = {
    Question:
      typeof qd.Question === 'string' ? qd.Question : '이 글에서 글쓴이가 주장하는 바로 가장 적절한 것은?',
...
async function main() {
  ...
  const outDir = path.resolve(PROJECT_ROOT, flags.get('out') || 'data/claim-finetune');
