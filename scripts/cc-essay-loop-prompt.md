...
## 난이도별 핵심 차이 (4 난도 모두 같은 지문 · 같은 두 문장 사용 가능하나 문법 포인트 겹침 최소화)
...
- 4 난도가 같은 문장 두 개를 쓰더라도, 각 난도의 `conditions` 와 `grammar_points` 가 **평가 포인트로 강조하는 문법 카테고리** 는 서로 다르게 분산할 것. (예: 기본=수동태, 중=관계대명사, 고=분사구문 축약, 최고=병렬 구조 — 같은 문장이지만 강조점 분리)
