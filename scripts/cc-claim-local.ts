/**
 * 로컬 주장 LoRA 추론 → prevalidate → (옵션) save.
 * Anthropic/Claude API 호출 없음.
 *
 *   Mac MLX:     npm run cc:claim-local -- --passage-id <ObjectId>
 *   Windows CUDA: npm run cc:claim-local -- --backend cuda --passage-id <ObjectId>
 *                 set CLAIM_BACKEND=cuda
 *   Pipeline:     npm run cc:claim-local -- --backend cuda --pipeline --passage-id <ObjectId>
 *
 *   npm run cc:claim-local -- --passage-id <ObjectId> --save
 *   npm run cc:claim-local -- --passage-file p.txt --textbook "..." --source "..."
 */
...
const INFER_MLX = path.join(PROJECT_ROOT, 'ml/claim/infer.py');
const INFER_CUDA = path.join(PROJECT_ROOT, 'ml/claim/windows/infer.py');
const PIPELINE_CUDA = path.join(PROJECT_ROOT, 'ml/claim/windows/pipeline_claim.py');
const ADAPTER_MLX = path.join(PROJECT_ROOT, 'ml/claim/adapters/claim-lora');
const ADAPTER_CUDA = path.join(PROJECT_ROOT, 'ml/claim/adapters/claim-lora-cuda');
...
function resolveBackend(flags: Map<string, string>): Backend {
  const raw = (flags.get('backend') || process.env.CLAIM_BACKEND || '').trim().toLowerCase();
...
function pythonBin(backend: Backend): string {
  if (backend === 'cuda') {
    const envPy = (process.env.CLAIM_CUDA_VENV || '').trim();
    const candidates = [
      envPy ? path.join(envPy, process.platform === 'win32' ? 'Scripts/python.exe' : 'bin/python') : '',
      path.join(PROJECT_ROOT, 'ml/claim/windows/.venv/Scripts/python.exe'),
      path.join(PROJECT_ROOT, 'ml/claim/windows/.venv/bin/python'),
    ].filter(Boolean);
  ...
  const envPy = (process.env.CLAIM_MLX_VENV || '').trim();
  const candidates = [
    envPy ? path.join(envPy, 'bin/python') : '',
    path.join(PROJECT_ROOT, 'ml/claim/.venv/bin/python'),
    '/tmp/claim-mlx-venv/bin/python',
  ].filter(Boolean);
  for (const p of candidates) {
    if (fs.existsSync(p)) return p;
  }
...
function runInfer(
  ...
  const tmp = path.join(DRAFTS_DIR, `_claim-passage-${Date.now()}.txt`);
...
async function main() {
  ...
  npm run cc:claim-local -- --passage-file p.txt --textbook "교재" --source "출처" [--save]

사전:
  npm run cc:claim-export
  Mac:    ml/claim/train.sh
  Windows: ml/claim/windows/setup.bat && train.bat
  Pipeline(CUDA): ml/claim/windows/ask_pipeline.bat
Claude/Anthropic 는 사용하지 않습니다. (backend=${backend}, platform=${process.platform})`);
  ...
  if (!fs.existsSync(adapter || defaultAdapter)) {
    console.error(
      `경고: adapter 디렉터리 없음 (${adapter || defaultAdapter}). 베이스 모델만으로 추론합니다.\n` +
        (backend === 'cuda'
          ? `학습: cd ml\\claim\\windows && train.bat`
          : `학습: cd ml/claim && ./train.sh`)
    );
  }
  ...
  if (typeof question_data.Category !== 'string') question_data.Category = '주장';

  const draftItem = {
    passage_id: pid || '000000000000000000000000',
    textbook: textbook || 'local',
    source: source || 'local-claim',
    type: '주장',
    option_type: 'English',
    status: '대기',
    ai_source: 'local-claim-lora',
    question_data,
  };
  ...
  const draftPath = path.join(
    DRAFTS_DIR,
    `claim-local-${pid || 'file'}-${stamp}.json`
  );
