 ...
 *
 * 사용:
 *   npx tsx scripts/import-grammar-problems.ts --user-id <ObjectId> --course "천일문 중등 WRITING LEVEL 1" \
 ...
 * - topicKey = writingTopicKey(과정, chapter, topic) — 문법 문제관리 트리와 정확히 일치해야 한다.
 */
...
import { WRITING_CURRICULA, writingTopicKey } from '../lib/writing-curriculum';
import { WRITING_PROBLEMS_COLLECTION, nextWritingSerial, formatWritingSerial } from '../lib/vip-writing-problem-bank';
...
async function main() {
  ...
  const course = opt('course') ?? '천일문 중등 WRITING LEVEL 1';
  const source = opt('source') ?? '천일문 WRITING';
  ...
  const cur = WRITING_CURRICULA.find((c) => c.과정 === course);
  if (!cur) fail(`과정 "${course}" 이 WRITING_CURRICULA 에 없음`);
  ...
  const col = db.collection(WRITING_PROBLEMS_COLLECTION);
  ...
  for (const pr of problems) {
    ...
    const topicKey = writingTopicKey(course, pr.chapter, pr.topic);
    ...
    if (existing) {
      ...
      const serialNo = await nextWritingSerial(db);
  ...
  console.log(`완료: 신규 ${inserted} (${formatWritingSerial(firstSerial)}~${formatWritingSerial(lastSerial)}), 갱신 ${updated}, 건너뜀 ${skipped}`);
  process.exit(0);
}
